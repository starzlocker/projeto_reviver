...
const ManualSteps = () => {
  ...
  const moveRight = () => {
    const manualStepsWrapper = document.querySelector(".manual-steps-wrapper"); // div wrapper, ela serve pra limitar o manual-steps com overflow hidden, pois ele tem width equivalente ao número de cards
    ...
    for (let i = 0; i < manualSteps.length; i++) {
      ...
      if (
        ...
        (newOffset && node.offsetLeft - newOffset <= wrapperW) // Se eu tiver encontrado uma nova posição inicial e os cards que vem depois CABEM todos dentro da nova exibição sem sobra
      ) {
        disableChevronRight();
      }
    }
  ...
  const disableChevronRight = () => {
      document.querySelector("#manual_steps_container .chevron-r").classList.add("disabled");
  }
  ...
  const moveLeft = () => {
    ...
    if (currentOffset) {
      for(let i = curOffset - 1; i >= 0; i--) { // começo da posição anterior ao meu ponto de inicio
        const node = manualSteps[i];
        ...
        if (
          ...
          curWidth += width;
        } else { // Quando eu encontrar um card cujo o offset esteja fora do meu espaço disponível, ai a gente desloca
          setCurOffset(i);
